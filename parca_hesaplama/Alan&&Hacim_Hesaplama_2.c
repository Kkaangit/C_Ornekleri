// Kullanıcı program bitirmeden yeni bir parça hesaplaması yaparken alan-hacim türü, geometrik şekil, uzunluk birimi türünü tekrardan seçer
// Dosya ismi kullanıcı tarafından belirlenir
#include <stdio.h>
#include <stdlib.h>
...
int main() {
    birim sayi;
    birim *birimler = NULL;
    Cins cins,*cinsler = NULL;;
    ikiBoyut ikiboyut, *Bellek_ikiboyut = NULL;
    üçBoyut ucboyut, *Bellek_ucboyut = NULL;
    int parca_sayisi = 0;
    ...
    do {
        ...
        cinsler = (Cins *)realloc(cinsler, sizeof(cins) * (parca_sayisi + 1));
        Bellek_ikiboyut = (ikiBoyut *)realloc(Bellek_ikiboyut, sizeof(ikiBoyut) * (parca_sayisi + 1));
        Bellek_ucboyut = (üçBoyut *)realloc(Bellek_ucboyut, sizeof(üçBoyut) * (parca_sayisi + 1));
        ...
        scanf("%d", &cins);
        if (cins == alan) {
            printf("sekil sec dotgenler icin 1'e daire icin 2'ye kure icin 3'e silindir icin 4'e basiniz\n");
            scanf("%d", &ikiboyut);
            if (ikiboyut != dörtgen && ikiboyut != daire && ikiboyut != küre_A && ikiboyut != silindir_A) {
                perror("yanlis veya hatali secim"); exit(1);
            }
        }else if (cins == hacim) {
            printf("sekil sec kup icin 1'e dirtdortgenprizma icin 2'ye kure icin 3'e basiniz silindir icin 4'e basiniz\n");
            scanf("%d", &ucboyut);
        ...
        cinsler[parca_sayisi] = cins;
        Bellek_ikiboyut[parca_sayisi] = ikiboyut;
        Bellek_ucboyut[parca_sayisi] = ucboyut;
    ...
    for (int i = 0; i < parca_sayisi; i++) {
        ...
        if (cinsler[i] == alan) {
            ...
            fprintf(toplam,"Seçilen geometrik şekil: %s\n\n",Dosya_2D(Bellek_ikiboyut[i]));
        }
        else if (cinsler[i] == hacim) {
            fprintf(toplam,"Hacim ölçümü yapıldı\n");
            fprintf(toplam,"Secilen geometrik şekil: %s\n\n", Dosya_3D(Bellek_ucboyut[i]));
        }
...
const char *Dosya_3D(üçBoyut geometrik) {
    switch (geometrik) {
        case küp: return "Küp";
        case dikdörtgen_prizma: "Dikdörtgen prizma";
